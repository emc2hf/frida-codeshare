/*
Hooks OpenSSL functions inside an Android app
Specifically, it intercepts SSL_write and BIO_write from libssl.so.

Monitors outgoing TLS traffic
Every time the app writes encrypted data, the script inspects the raw bytes being sent.

Identifies the TLS ClientHello packet
It checks if the intercepted data is a TLS handshake message that starts the HTTPS connection.

Parses the ClientHello fields
Once a ClientHello is detected, the script extracts:
...
Prints the full TLS configuration the app is using
It dumps a JSON summary showing everything the client advertises to the server.

Only dumps once
After capturing the first ClientHello, it stops parsing further data.
*/
